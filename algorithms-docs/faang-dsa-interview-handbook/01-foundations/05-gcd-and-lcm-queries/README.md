...
## 3. Problem Statement
Return gcd and lcm for each pair.
...
Apply the `math` pattern to avoid repeated work and reduce asymptotic cost.
...
```java
class Solution { long gcd(long a,long b){ while(b!=0){ long t=a%b; a=b; b=t;} return Math.abs(a);} long[] solve(long a,long b){ long g=gcd(a,b); return new long[]{g,(a/g)*b}; } }
```
...
Core mathematical reduction pattern.
