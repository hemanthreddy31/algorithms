...
## 3. Problem Statement
Compute x^n for signed integer n.
...
Apply the `recursion` pattern to avoid repeated work and reduce asymptotic cost.
...
```java
class Solution { public double solve(double x,int n){ long p=n; if(p<0){x=1.0/x; p=-p;} double ans=1; while(p>0){ if((p&1)==1) ans*=x; x*=x; p>>=1;} return ans; } }
```
