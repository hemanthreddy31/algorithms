...
Apply the `hashing` pattern to avoid repeated work and reduce asymptotic cost.
...
```java
import java.util.*; class Solution { public int solve(int[] nums){ Set<Integer> s=new HashSet<>(); for(int x:nums) s.add(x); int best=0; for(int x:s) if(!s.contains(x-1)){ int y=x; while(s.contains(y)) y++; best=Math.max(best,y-x);} return best;} }
```
